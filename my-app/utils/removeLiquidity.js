...
export const removeLiquidity = async (signer, removeLPTokensWei) => {
  ...
  const tx = await exchangeContract.removeLiquidity(removeLPTokensWei, MY_BOYS_DEVS_TOKEN_ADDRESS);
  await tx.wait();
};

/**
 * getTokensAfterRemove: Calculates the amount of `Eth` and `MBD` tokens
...
export const getTokensAfterRemove = async (
  ...
  try {
    ...
    // Get the total supply of `MyBoysDevs` LP tokens
    ...
    // Similarly we also maintain a ratio for the `MBD` tokens, so here in our case
    // Ratio is -> (amount of MBD tokens sent back to the user / MBD Token reserve) = (LP tokens withdrawn) / (total supply of LP tokens)
    // Then (amount of MBD tokens sent back to the user) = (MBD token reserve * LP tokens withdrawn) / (total supply of LP tokens)
    const _removeEther = _ethBalance.mul(removeLPTokenWei).div(_totalSupply);
    const _removeMBD = myBoysDevsTokenReserve
